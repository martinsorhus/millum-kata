...
test('filters out all users with no id', () => {
    ...
    expect(first([...users, ...additionalUsers])).not.toContain({ name: 'Johnny', role: 'regular' })
    expect(first([...users, ...additionalUsers])).not.toContain({ name: 'OnkelP', role: 'regular' })
    expect(first([...users, ...additionalUsers])).not.toContain({ name: 'Eirik', role: 'superadmin' })
    expect(first([...users, ...additionalUsers])).not.toContain({ name: 'Stian', role: 'admin' })
    expect(first([...users, ...additionalUsers])).not.toContain({ name: 'Egil', role: 'regular' })
    expect(first([...users, ...additionalUsers])).not.toContain({ name: 'Andrea', role: 'superadmin' })
